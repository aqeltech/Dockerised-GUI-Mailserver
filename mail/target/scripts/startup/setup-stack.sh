...
function _setup_dovecot_quota
{
    ...
    if [[ ${ENABLE_LDAP} -eq 1 ]] || [[ ${ENABLE_SQL} -eq 1 ]] || [[ ${SMTP_ONLY} -eq 1 ]] || [[ ${ENABLE_QUOTAS} -eq 0 ]]
    then
...
function _setup_dovecot_local_user
{
  ...
  then
    if [[ ${ENABLE_LDAP} -eq 0 ]] || [[ ${ENABLE_SQL} -eq 0 ]]
    then
      _shutdown 'Unless using LDAP or SQL, you need at least 1 email account to start Dovecot.'
    fi
  fi
}
...
function _setup_sql() {
  ...
  _notify 'task' 'Setting up SQL Mapping'

  for i in 'users' 'groups' 'aliases' 'domains'; do
    local FPATH="/tmp/docker-mailserver/${i}.cf"
    if [[ -f ${FPATH} ]]; then
      cp "${FPATH}" "/etc/postfix/sql/${i}.cf"
    fi
  done

  # Configuring Postfix
  _notify 'inf' "Overriding Postfix's SQL Configuration Files."

  local FILES=(
    /etc/postfix/sql/domains_maps.cf
    /etc/postfix/sql/mailbox_maps.cf
    /etc/postfix/sql/alias_domain_mailbox_maps.cf
    /etc/postfix/sql/alias_maps.cf
    /etc/postfix/sql/alias_domain_maps.cf
    /etc/postfix/sql/alias_domain_catchall_maps.cf
  )

  declare -A _postfix_sql_mapping
  _postfix_sql_mapping["SQL_HOSTS"]="${SQL_HOSTS:="${SQL_HOSTS}"}"
  _postfix_sql_mapping["SQL_DBNAME"]="${SQL_DBNAME:="${SQL_DBNAME}"}"
  _postfix_sql_mapping["SQL_USER"]="${SQL_USER:="${SQL_USER}"}"
  _postfix_sql_mapping["SQL_PASSWORD"]="${SQL_PASSWORD:="${SQL_PASSWORD}"}"

  for VAR in "${!_postfix_sql_mapping[@]}"; do
    export "${VAR}=${_postfix_sql_mapping[${VAR}]}"
  done

  for FILE in "${FILES[@]}"; do
    configomat.sh "SQL_" "${FILE}"
  done

  # Configuring Dovecot
  _notify 'inf' "Overriding Dovecot's SQL Configuration File."
  # trap "set +x; sleep 5; set -x" DEBUG
  declare -A _dovecot_sql_mapping
  _dovecot_sql_mapping["SQL_DRIVER"]="${SQL_DRIVER:="${SQL_DRIVER}"}"
  # _dovecot_sql_mapping["SQL_HOSTS"]="${SQL_HOSTS:="${SQL_HOSTS}"}"
  # _dovecot_sql_mapping["SQL_DBNAME"]="${SQL_DBNAME:="${SQL_DBNAME}"}"
  # _dovecot_sql_mapping["SQL_USER"]="${SQL_USER:="${SQL_USER}"}"
  # _dovecot_sql_mapping["SQL_PASSWORD"]="${SQL_PASSWORD:="${SQL_PASSWORD}"}"
  _dovecot_sql_connect="host=${SQL_HOSTS} dbname=${SQL_DBNAME} user=${SQL_USER} password=${SQL_PASSWORD}"
  _dovecot_sql_mapping["SQL_DEFAULT_PASS_SCHEME"]="${SQL_PASS_SCHEME:="${SQL_PASS_SCHEME}"}"

  for VAR in "${!_dovecot_sql_mapping[@]}"; do
    export "${VAR}=${_dovecot_sql_mapping[${VAR}]}"
  done

  configomat.sh "SQL_" "/etc/dovecot/dovecot-sql.conf.ext"
  # Temporary Fix to override an array of variables
  _dovecot_sql_connect_escaped=$(sed 's/[&/\]/\\&/g' <<<"$_dovecot_sql_connect")
  sed -i "s/^connect.*/connect = $_dovecot_sql_connect_escaped/g" "/etc/dovecot/dovecot-sql.conf.ext"
  sed -i "s/^connect.*/connect = $_dovecot_sql_connect_escaped/g" "/etc/dovecot/dovecot-dict-sql.conf.ext"


  _notify 'inf' "Enabling Dovecot SQL authentification"
  sed -i -e '/\!include auth-sql\.conf\.ext/s/^#//' /etc/dovecot/conf.d/10-auth.conf
  sed -i -e '/\!include auth-passwdfile\.inc/s/^/#/' /etc/dovecot/conf.d/10-auth.conf

  _notify 'inf' "Configuring dovecot to utilise sql for quota"
  if [[ ${ENABLE_QUOTAS} -eq 1 ]]
    then
      sed -i \
        "s|quota = count:User quota|quota = dict:User quota::proxy::quota|g" \
        /etc/dovecot/conf.d/90-quota.conf
      sed -i \
        "s|# mode = 0600|mode = 0600|g" \
        /etc/dovecot/conf.d/10-master.conf
      sed -i \
        "s|# user = docker|user = docker|g" \
        /etc/dovecot/conf.d/10-master.conf
      sed -i \
        "s|# group = docker|group = docker|g" \
        /etc/dovecot/conf.d/10-master.conf
      sed -i \
        "s|#quota = |quota = |g" \
        /etc/dovecot/dovecot.conf
  ...
  # Mapping postfix to use SQL Configurations
  _notify 'inf' "Mapping Postfix to SQL Configuration Files"
  if [[ -f /etc/postfix/sql/domains_maps.cf ]]; then
    postconf -e "virtual_mailbox_domains= proxy:${SQL_DRIVER}:/etc/postfix/sql/domains_maps.cf" ||
      _notify 'inf' "==> Warning: One or All of the virtual_mailbox_domains files are not found"
  fi
  if [ -f /etc/postfix/sql/mailbox_maps.cf ] && [ -f /etc/postfix/sql/alias_domain_mailbox_maps.cf ]; then
    postconf -e "virtual_mailbox_maps= proxy:${SQL_DRIVER}:/etc/postfix/sql/mailbox_maps.cf, proxy:${SQL_DRIVER}:/etc/postfix/sql/alias_domain_mailbox_maps.cf" ||
      _notify 'inf' "==> Warning: One or All of the virtual_mailbox_maps files are not found"
  fi
  if [ -f /etc/postfix/sql/alias_maps.cf ] && [ -f /etc/postfix/sql/alias_domain_maps.cf ] && [ -f /etc/postfix/sql/alias_domain_catchall_maps.cf ]; then
    postconf -e "virtual_alias_maps= proxy:${SQL_DRIVER}:/etc/postfix/sql/alias_maps.cf,proxy:${SQL_DRIVER}:/etc/postfix/sql/alias_domain_maps.cf,proxy:${SQL_DRIVER}:/etc/postfix/sql/alias_domain_catchall_maps.cf" ||
      _notify 'inf' "==> Warning: One or All of the virtual_alias_maps files are not found"
  fi
