...
VARS[ENABLE_SQL]="${ENABLE_SQL:=0}"
VARS[SQL_DRIVER]="${SQL_DRIVER:=mysql}"
...
VARS[SQL_PASS_SCHEME]="${SQL_PASS_SCHEME:=MD5-CRYPT}"
...
function register_functions
{
  ...
  [[ ${ENABLE_SQL} -eq 1 ]] && _register_setup_function '_setup_sql'
  ...
  [[ ${ENABLE_SQL} -eq 0 ]] && _register_start_daemon '_start_changedetector'
